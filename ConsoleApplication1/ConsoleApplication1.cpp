...
//Нахождение индекса первого вхождения минимального значения среди 
//отрицательных чисел массива (статический, динамический массив)
int func1static(int mas[], int n) {
    ...
    if (min_i != -1)
        return min_i; //возврат индекса
    else
        return -1; //если нет отрицательных чисел
}

//Нахождение индекса первого вхождения минимального значения среди 
//отрицательных чисел массива (векторный массив)
...
//Вставка нового элемента массива после минимального элемента массива
//(статический массив)
...
int func2vector(vector<int>& vec, int newelem) {
    ...
    //вставка нового элемента в векторе
...
void func3dynamic(int*& mas, int& n, int k)
{
    ...
    for (int i = 0; i < n; i++)
        if (k != -1 && mas[i] == p) {
            ...
            i--;
            n--;
            mas = (int*)realloc(mas, n * sizeof(int)); //освобождение памяти
        }
}
...
void func3vector(vector<int>& vec, int v) {
    if (v != -1)
    {
        ...
        for (int i = 0; i < vec.size(); i++)
            if (vec[i] == min_znach) {
                vec.erase(vec.begin() + i); //удаление элемента по индексу
                i--;
            }
    }
    else
...
int main()
{
    ...
    do {
        cout << "Практическая работа №1 «Одномерный массив» Кузнецов Андрей ИКБО-09-21\nКакой тип массива будем использовать?\n1: Static, 2: Dynamic, 3: Vector\nВвод: ";
        cin >> menu;
        switch (menu)
        {
